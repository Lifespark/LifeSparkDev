/*<Material>
  ...
  <var name="gReciprocalFrame" type="string" usage="texture" default="" texcoord="0"/>
  </Params>
  ...
  <fragshader entry = "fsluminancepass"/>
  </Pass>
  </Technique>
...
void fsluminancepass()
{
	vec3 luminanceFactor = vec3(0.299f, 0.587f, 0.114f);
	vec3 scene = texture2D(gInputSampler, tex).rgb;
	float luminance = dot(scene, luminanceFactor);
	
	gl_FragColor = vec4(scene, luminance);
}
...
void fsfxaapass()
{
	...
	gl_FragColor = FxaaPixelShader(pos, temp, gInputSampler, gInputSampler, gInputSampler, gReciprocalFrame, temp, temp, temp, 1.0, 0.063, 0.0625, 4.0, 0.125, 0.05, temp);
}
