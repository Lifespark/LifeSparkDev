...
//#endif
    ...
    fogParam = vec2(0.0, 0.0);
#ifdef FOG_DIST
    fogParam.x = clamp(((gl_Position.w - g_FogParam.x)/(g_FogParam.y - g_FogParam.x)), 0.0, 1.0);
#endif

#ifdef FOG_HEIGHT
	float posy = dot(g_World[1],vec4(vPos,1.0));
	fogParam.y = clamp((posy - g_FogParam.z)/(g_FogParam.w - g_FogParam.z),0.0,1.0);
#endif

// for dof
   float distScale = 1.0;//1.0;//0.02;
   viewVec = -vec3(g_WorldView * vec4(vPos,1.0)) * distScale;
...
#ifdef SPEC_ON
	SpecColor = texture2D(g_SpecularTex, tex).xyz;
	SpecColor *= specular;
#endif
...
#ifdef FOG_HEIGHT
    light.xyz = mix(light.xyz, g_HeightFogColor.xyz, fogParam.y);
#endif

#ifdef FOG_DIST
    light.xyz = mix(light.xyz, g_DisFogColor.xyz, fogParam.x);
#endif
